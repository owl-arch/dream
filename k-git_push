...
# Verifica se a pasta foi inicialida como repositório Git
if [ ! -d .git ]; then
  echo "Inicializando repositório Git" >>$logfile
  git init >/dev/null 2>&1
fi

# Adiciona todos os alterados
echo "1. Adiciona todos os arquivos alterados" >>$logfile 
git add . >/dev/null 2>&1

# Documenta a atualização
echo "2. Documenta a atualização" >>$logfile 
git commit -m "Update by $node in $now" >/dev/null 2>&1

# Configura a 'branch' (ramo)
if git rev-parse --verify HEAD >/dev/null 2>&1; then
  echo "3. Configura a branch para 'main'" >>$logfile 
...
echo "4. Adiciona pasta local no remote" >>$logfile 
...
echo "5. Atualiza o projeto no repositório do github" >>$logfile 
git push -uf $1 main # >/dev/null 2>&1

# Remove pasta do remote
echo "6. Remove pasta local do remote" >>$logfile 
